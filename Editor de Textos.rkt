...
;Representa las caracteristicas del texto (contenido, tamaño de la fuente y color) mediante la estructura "texto" y un color con un string.
...
(check-expect (cambiaColor (make-texto "-mundo" "indigo" 20) "5") (make-texto "-mundo" "indigo" 20))

;Representa las caracteristicas del texto (contenido, tamaño de la fuente y color) con la estructura "texto" y la tecla precionada con un string.
...
(define (cambiaTamañoFuente n k)
  (cond[(key=? k "up") (make-texto (texto-cadena n) (texto-color n) (+ (texto-tam n) 1))]
       [(key=? k "down") (make-texto (texto-cadena n) (texto-color n) (if (< 1 (- (texto-tam n) 1)) (- (texto-tam n) 1) (texto-tam n)))]))
...
;Representa las caracteristicas del texto (contenido, tamaño de la fuente y color) con la estructura "texto" y la pantalla del editor de texto mediante una imagen.
...
;Representa las características del texto (contenido, tamaño de la fuente y color) con la estructura "texto" y la tecla precionada con un string.
;agregarCaracter : texto string -> texto
;Agrega los caracteres digitados a la cadena, borra caracteres con la tecla backspace (en el caso que la cadena no sea vacía), determina el tamanho de fuente con "up" y "down" y cambia el color de la cadena.
(define (agregarCaracter n k)
  (cond
    ;Determina el tamaño de la fuente
    [(or (key=? k "up") (key=? k "down")) (cambiaTamañoFuente n k)]
...
(check-expect (agregarCaracter (make-texto "" "indigo" 20) "a") (make-texto "a" "indigo" 20))
(check-expect (agregarCaracter (make-texto "Buenas" "indigo" 20) " ") (make-texto "Buenas " "indigo" 20))
(check-expect (agregarCaracter (make-texto "Winter is Coming" "indigo" 20) "f1") (make-texto "Winter is Coming" "black" 20))
(check-expect (agregarCaracter (make-texto "qsy" "indigo" 20) "\b") (make-texto "qs" "indigo" 20))
(check-expect (agregarCaracter (make-texto "" "indigo" 20) "\b") (make-texto "" "indigo" 20))
